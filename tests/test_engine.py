...
# 외부 채널 하드 캡
check("외부 × D0 → A4", decide(sec(0), EXTERNAL, POLICY).mode, 4)
check("외부 × D1 → A0", decide(sec(1), EXTERNAL, POLICY).mode, 0)
check("외부 × D2 (판단 질의여도) → A0", decide(sec(2), EXTERNAL, POLICY, "judgment").mode, 0)

# D0는 누구에게나 공개
check("신입 × D0 → A4", decide(sec(0), JUNIOR, POLICY).mode, 4)
...
check("영업팀원 C2 × D3 영업 데이터 (gap -1, 부서 일치) → A3",
      decide(sec(3, ["영업팀"]), SALES_REP, POLICY).mode, 3)
...
check("신입 C1 × D2 판단 질의 (gap -1) → A3", decide(sec(2), JUNIOR, POLICY, "judgment").mode, 3)
...
decisions = [
    (s_restricted, Decision(3, 1, 3, 1, -2)),
    (s_allowed, Decision(4, 4, 2, 1, -1)),
]
fb = forbidden_strings(decisions)
assert fb == ["94%"], fb  # 미래금융은 A4 섹션에서 허용되므로 제외
