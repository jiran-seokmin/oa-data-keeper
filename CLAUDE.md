...
- **분류 결과는 고정·커밋**: 등급 시드는 `app/seed_db.py`의 `GRADES`(코드에 커밋된 정적 데이터)이며 `python -m app.init_samples --reset`을 명시적으로 실행할 때만 `data/samples/*.md`를 SQLite(`datakeeper.db`, gitignore 대상)에 시딩한다. 서버 실행·일반 CLI 실행만으로 samples를 보안 등급 분류하거나 DB에 넣지 않는다. 데모 재현성을 위해 데모 직전에 라이브 분류를 돌리지 않는다. 샘플 문서(`data/samples/`)를 수정하면 GRADES의 해당 heading 키(`<파일stem>#<heading인덱스>`)도 함께 갱신해야 한다 (미매칭은 D4 격리, `python -m app.init_samples --report`로 점검).
